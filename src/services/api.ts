...
class ApiService {
  ...
  async getAllTransactions(params: { page: number; limit: number; search: string; transactionTypeId?: number; transactionModeId?: number; status?: string }) {
    ...
    if (params.transactionTypeId) {
      queryParams.append('transactionTypeId', params.transactionTypeId.toString());
    }
    if (params.transactionModeId) {
      queryParams.append('transactionModeId', params.transactionModeId.toString());
    }
    if (params.status) {
      queryParams.append('status', params.status);
    }
  ...
  async getPendingTransactions(params: { 
    ...
    if (params.transactionModeFilter && params.transactionModeFilter !== -1) {
      queryParams.append('transactionModeFilter', params.transactionModeFilter.toString());
    } else {
      queryParams.append('transactionModeFilter', '-1');
    }

    if (params.transactionalBankFilter) {
      queryParams.append('transactionalBankFilter', params.transactionalBankFilter);
    } else {
      queryParams.append('transactionalBankFilter', '');
    }

    if (params.dateYYYYMMddFilter) {
      queryParams.append('dateYYYYMMddFilter', params.dateYYYYMMddFilter);
    } else {
